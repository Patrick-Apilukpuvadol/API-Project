...
def add_customer_log():
    ...
    return customer_tour_booking_schema.dump(customer_log), 201 # Returns result 

@customer_tour_bp.route('/<int:customer_id>', methods=['DELETE'])
@jwt_required()
def delete_customer_log(customer_id): #deletes the customer log 
    
    admin_status = admin_authorised() # authorise_admin function 
    if not admin_status:
        return {'error': 'You do not possess Admin rights to delete this customer log'} #  return this error message if not admin
    stmt = db.select(Customer_tour_booking).filter_by(customer_id=customer_id) # queries and searches for id
    customer_log = db.session.scalar(stmt)
    if customer_log:
